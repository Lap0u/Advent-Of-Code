...
const checkSplittedGame = (splittedGame) => {
  ...
  let blueCube = redCube = greenCube = 1
  for (let i = 0; i < splittedCubes.length; i++) {
    const splittedCube = splittedCubes[i].trim(' ').split(' ')
    if (splittedCube[1] === 'red') redCube = Math.max(splittedCube[0], redCube)
    if (splittedCube[1] === 'green') greenCube = Math.max(splittedCube[0], greenCube)
    if (splittedCube[1] === 'blue') blueCube = Math.max(splittedCube[0], blueCube)
  }
  return { blueCube, redCube, greenCube }
}

const checkIfGamePossible = (game) => {
  const splittedGames = game.split(';')
  let maxBlue = maxRed = maxGreen = 1

  for (let i = 0; i < splittedGames.length; i++) {
    let { blueCube: tempBlue, redCube: tempRed, greenCube: tempGreen } = checkSplittedGame(splittedGames[i])
    maxBlue = Math.max(tempBlue, maxBlue)
    maxRed = Math.max(tempRed, maxRed)
    maxGreen = Math.max(tempGreen, maxGreen)
  }
...
const countPossibleGames = () => {
  ...
  for (let i = 0; i < input.length; i++) {
    const { maxBlue, maxRed, maxGreen } = checkIfGamePossible(input[i][1])
