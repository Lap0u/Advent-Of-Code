...
const findDigitRegex = /\d/
const decodeInput = (input) => {
  let firstDigit = -1
  let lastDigit = -1
  for (let j = 0; j < input.length; j++) {
    if (firstDigit === -1 && findDigitRegex.test(input[j])) {
      firstDigit = input[j]
    }
    if (lastDigit === -1 && findDigitRegex.test(input[input.length - 1 - j])) {
      lastDigit = input[input.length - 1 - j]
    }
  }
